...
using inner_kernel_type = sventt::IterativeNTT<
    ...
    sventt::RadixFourScalarLayer<sventt::PAdic64Scalar<modulus_type>,
                                 std::uint64_t{1} << 9, std::uint64_t{1} << 8>,
    ...
    sventt::RadixFourScalarLayer<sventt::FixedPoint64Scalar<modulus_type>,
                                 std::uint64_t{1} << 9, std::uint64_t{1} << 2,
                                 m>>;

using kernel_type = sventt::RecursiveNTT<
    modulus_type, m,
    sventt::RadixFourScalarLayer<sventt::PAdic64Scalar<modulus_type>, m, m>,
    inner_kernel_type>;

const std::string name{"recursive, scalar, radix-2,4"};
